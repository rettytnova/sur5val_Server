...
// 게임 시작 함수 호출
export const gameStartHandlerTwo = async (socket: CustomSocket, payload: Object) => {
  ...
  try {
    ...
    if (user !== undefined) {
      ...
      for (let i = 0; i < normalRound; i++) {
        await normalPhaseNotification(i + 1, room.getRoomId(), inGameTime * i);
      }
      await bossPhaseNotification(normalRound + 1, room.getRoomId(), inGameTime * normalRound);
      inGameTimeSessions[room.getRoomId()] = Date.now();

      // 게임 종료 notification 보내기
      setTimeout(
        async () => {
          await gameEndNotification(room.getRoomId(), 4);
        },
...
export const normalPhaseNotification = async (level: number, roomId: number, sendTime: number) => {
  setTimeout(async () => {
    ...
    const characterPositionDatas = Server.getInstance().getPositions();
    ...
    await userUpdateNotification(roomData);
  }, sendTime);
};
