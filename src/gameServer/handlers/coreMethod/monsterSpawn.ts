...
export const monsterSpawnStart = (roomId: number, level: number, bossIdx: number) => {
    try {
        ...
        // 공격팀 유저 마나 회복시키기
        for (let i = 0; i < room.getUsers().length; i++) {
            const character = characterDB.find((data) => data.characterType === room.getUsers()[i].getCharacter().characterType);
            if (character) {
        ...
        let characterPositionDatas = Server.getInstance().getPositions().find((position) => position.getPositionRoomId() === roomId)
...
const monsterSpawn = (roomId: number, level: number, positionList: SpawnPositionData[]) => {
    ...
    // 랜덤 위치 생성하기
    const characterPositionDatas = Server.getInstance().getPositions().find((position) => position.getPositionRoomId() === room.getRoomId())
    if (!characterPositionDatas) {
