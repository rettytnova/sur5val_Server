...
export const joinRandomRoomHandler = async (socket: CustomSocket) => {
  ...
  if (!user) {
    console.error('joinRandomRoomHandler user 없음');
    sendPacket(socket, config.packetType.JOIN_RANDOM_ROOM_RESPONSE, joinRandomFailSendData);
    return;
  }
  ...
  if (isAleadyJoinRoom) {
    console.error(`joinRandomRoomHandler 이미 참여중인 방이 존재합니다. id ${isAleadyJoinRoom.getRoomId()}`);
    sendPacket(socket, config.packetType.JOIN_RANDOM_ROOM_RESPONSE, joinRandomFailSendData);
  } else {
    ...
    Server.getInstance().chattingServerSend(config.chattingPacketType.CHATTING_JOIN_ROOM_REQUEST, {
      email: user.getEmail(),
      ownerEmail: randomRoom.getRoomOwnerEmail()
    });
