...
import { CreateRoomPayload, CustomSocket, Room, RoomTwo, User } from '../../interface/interface.js';
...
export const createRoomHandlerTwo = async (socket: CustomSocket, payload: Object) => {
    ...
    if (!user) {
        ...
        if (!createRoomPayload.name || !createRoomPayload.maxUserNum) {
            ...
            Server.getInstance().chattingServerSend(
                config.chattingPacketType.CHATTING_CREATE_ROOM_REQUEST, { email: user.getEmail() });
        }
