...
export const gameEndNotification = (roomId: number, winRType: number) => {
    ...
    // 몬스터를 room에서 삭제
    const remainUsers = endRoom.getUsers().filter((user: UserSessions) => user.getCharacter().roleType !== RoleType.WEAK_MONSTER);
    ...
    const remainCharacterPositions = characterPositions.filter((characterPosition: PositionSessions) => characterPosition.getPositionRoomId() !== roomId);
    Server.getInstance().setPositions(remainCharacterPositions);

    const fleaMarketCards = Server.getInstance().getMarkets();
    const remainFleaMarketCards = fleaMarketCards.filter((fleaMarketCard: MarketSessions) => fleaMarketCard.getRoomId() !== roomId);
    ...
    const remainEndUser = endRoom.getUsers().filter((user) => userIds.includes(user.getId()));
    ...
    if (roomUsers.length === 0) {
        const newRooms = Server.getInstance().getRooms().filter((room) => room.getRoomId() === roomId)
        Server.getInstance().setRooms(newRooms)
    }
    else {
        const isHaveOwner = endRoom.getUsers().some((endRoomUser: UserSessions) => endRoomUser.getId() === endRoom.getRoomOwnerId());
        if (!isHaveOwner) {
