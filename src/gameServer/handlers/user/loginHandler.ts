...
export const loginHandler = async (socket: CustomSocket, payload: Object): Promise<void> => {
    ...
    try {
        ...
        console.info(`로그인 성공 id : ${userByEmail.id} email : ${userByEmail.email} 로그인 중인 클라 ${Server.getInstance().getUsers().length}`);
    } catch (error) {
    ...
    const room = rooms.find((room: GameRoom) => room.getUsers().some((roomUser: UserSessions) => roomUser.getId() === userByEmail.id));
    if (room) {
        ...
        if (characterPositions.length > 0) {
            const characterPosition = characterPositions.find((characterPosition: PositionSessions) => characterPosition.getPositionRoomId() === room.getRoomId());
