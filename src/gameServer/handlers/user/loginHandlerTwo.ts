 ...
 */
export const loginHandlerTwo = async (socket: CustomSocket, payload: Object): Promise<void> => {
    ...
    try {
        ...
        console.info(`로그인 성공 id : ${userByEmail.id} email : ${userByEmail.email}`);
    } catch (error) {
    ...
    const room = rooms.find((room: GameRoom) => room.getUsers().find((roomUser: UserSessions) => roomUser.getId() === userByEmail.id));
    if (room) {
