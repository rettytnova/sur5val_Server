...
import { monsterRewardTwo, setCardRewardsTwo, setStatRewardsTwo } from '../coreMethod/monsterRewardTwo.js';
 ...
 */
export const useCardHandlerTwo = async (socket: CustomSocket, payload: Object): Promise<void> => {
  // response 데이터 초기화 ----------------------------------------------------------------------
  ...
  try {
    // redisUser 정보 찾기
    const userData = getUserBySocket(socket);
    if (!userData) {
    ...
    const room: GameRoom | undefined = Server.getInstance()
      .getRooms()
      .find((room) => room.getRoomId() === userData.getId());
    if (!room) {
    ...
    // room 의 user정보 찾기
    let user: UserSessions | null = null;
    for (let i = 0; i < room.getUsers().length; i++) {
      if (room.getUsers()[i].getId() === userData.getId()) {
    ...
    if (user.getCharacter().roleType === RoleType.SUR5VAL) {
      ...
      // 공격 가능 여부 확인
      for (let i = 0; i < characterPositions.getCharacterPosition().length; i++) {
        let userPos: CharacterPositionData = characterPositions.getCharacterPosition()[i];
        if (user !== null && userPos.id === user.getId()) {
          if (
            ...
            for (let j = 0; j < characterPositions.getCharacterPosition().length; j++) {
              let characterPos: CharacterPositionData = characterPositions.getCharacterPosition()[j];
...
const attackRagne = (
  ...
  const positionDatas: CharacterPositionData[] | undefined = characterPositions.getCharacterPosition();
  ...
  for (let i = 0; i < alivePlayers.length; i++) {
    // 공격 가능 여부 확인
    for (let j = 0; j < characterPositions.getCharacterPosition().length; j++) {
      let attackerPos: CharacterPositionData = characterPositions.getCharacterPosition()[j];
      if (attacker !== null && attackerPos.id === attacker.getId()) {
        if (
          ...
          for (let k = 0; k < characterPositions.getCharacterPosition().length; k++) {
            let characterPos: CharacterPositionData = characterPositions.getCharacterPosition()[k];
...
const attackTarget = (attackerId: number, roomId: number, skillCoeffcient: number, targetId: number) => {
  ...
  if (target.getCharacter().aliveState && target.getCharacter().hp <= 0) {
    ...
    monsterRewardTwo(nowRooms, nowRoom, attacker, target);
  }

  // 보스를 죽였는지 검사
  if (target.getCharacter().roleType === RoleType.BOSS_MONSTER && target.getCharacter().hp <= 0) {
    gameEndNotificationTwo(roomId, 3);
    return;
  }
  ...
  userUpdateNotificationTwo(nowRoom);
};
...
const summonSpiritBuff = async (
  ...
  const attackSkill = setInterval(async () => {
    ...
    const characterPositionData = positionSessions.getCharacterPosition();
    ...
    for (let i = 0; i < targetMonsters.length; i++) {
      ...
      if (target.hp <= 0 && target.aliveState) {
        ...
        await monsterRewardTwo(nowRooms, nowRoom, skillUser, targetMonsters[i].monster);
      }

      // 보스를 죽였는지 검사
      if (target.roleType === RoleType.BOSS_MONSTER && target.hp <= 0) {
        await gameEndNotificationTwo(nowRoom.getRoomId(), 3);
        return;
      }
    }
    lastAttack = Date.now();
    await userUpdateNotificationTwo(nowRoom);
  }, 200);
...
const usePotion = (user: UserSessions, room: GameRoom, cardsType: number) => {
  ...
  while (user.getCharacter().exp >= user.getCharacter().maxExp) {
    ...
    // 직업별 스탯 증가
    if (!setStatRewardsTwo(user)) {
      console.error('setStatRewards 실패');
      return false;
    }
    // 직업별 카드 보상 지급
    else if (!setCardRewardsTwo(user)) {
